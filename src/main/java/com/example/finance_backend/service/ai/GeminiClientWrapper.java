...
/**
 * Lớp bao bọc (Wrapper) cho SDK Google Gemini AI.
 * Trực tiếp tương tác với API của Google để phân tích ngôn ngữ tự nhiên.
 */
@Component
@Slf4j
public class GeminiClientWrapper {
     ...
     * @param base64Image  Ảnh đính kèm (nếu có).
     * @param history      Lịch sử trò chuyện để giữ ngữ cảnh.
     * @param language     Ngôn ngữ mong muốn.
     * @param customApiKey Key cá nhân của user.
     * @return GeminiParseResult Đối tượng chứa thông tin đã bóc tách.
     */
    public GeminiParseResult parse(String message, String base64Image, List<AiMessage> history, String language,
            String customApiKey) {
        int maxRetries = 2; // Thử lại tối đa 2 lần nếu gặp lỗi tạm thời (như 429)
        int attempt = 0;

        while (attempt <= maxRetries) {
            try {
                // Xây dựng các thành phần của Request
                ...
                List<Tool> tools = buildTools(); // Khai báo các function có thể gọi
                ...
                GenerateContentResponse response = getClient(customApiKey).models.generateContent(model, userContents,
                        config);
                ...
                if (response.candidates() != null && response.candidates().isPresent()
                    ...
                    if (candidate.content() != null && candidate.content().isPresent()) {
                        ...
                        if (content.parts() != null && content.parts().isPresent()) {
                            // 1. Kiểm tra xem Gemini có yêu cầu gọi Function không
                            for (Part part : content.parts().get()) {
                                if (part.functionCall() != null && part.functionCall().isPresent()) {
                                    return mapFunctionCallToResult(part.functionCall().get());
                                }
                            }
                            // 2. Nếu không gọi function, kiểm tra xem có văn bản chat bình thường không
                            for (Part part : content.parts().get()) {
    ...
    /**
     * Chuyển đổi yêu cầu gọi hàm (Function Call) của AI sang đối tượng DTO nội bộ.
     */
    private GeminiParseResult mapFunctionCallToResult(FunctionCall call) {
        ...
        // Dựa vào tên hàm AI muốn gọi để bóc tách tham số tương ứng
        switch (funcName) {
            case "insertTransaction":
                res.setIntent("INSERT");
                GeminiParsedEntry entry = new GeminiParsedEntry();
                if (args.containsKey("amount"))
                    entry.setAmount(new BigDecimal(args.get("amount").toString()));
                if (args.containsKey("categoryName"))
                    entry.setCategoryName((String) args.get("categoryName"));
                if (args.containsKey("note"))
                    entry.setNote((String) args.get("note"));
                if (args.containsKey("type"))
                    entry.setType((String) args.get("type"));
                if (args.containsKey("date"))
                    entry.setDate((String) args.get("date"));
                if (args.containsKey("walletName"))
                    entry.setWalletName((String) args.get("walletName"));
            ...
            case "updateTransaction":
                ...
                if (args.containsKey("amount"))
                    updateTarget.setAmount(new BigDecimal(args.get("amount").toString()));
                if (args.containsKey("noteKeywords"))
                    updateTarget.setNoteKeywords((String) args.get("noteKeywords"));
                if (args.containsKey("date"))
                    updateTarget.setDate((String) args.get("date"));
                if (args.containsKey("categoryName"))
                    updateTarget.setCategoryName((String) args.get("categoryName"));
                res.setTarget(updateTarget);

                GeminiParsedEntry updateData = new GeminiParsedEntry();
                if (args.containsKey("amount"))
                    updateData.setAmount(new BigDecimal(args.get("amount").toString()));
            ...
            case "deleteTransaction":
                ...
                if (args.containsKey("deleteAll"))
                    delTarget.setDeleteAll((Boolean) args.get("deleteAll"));
                if (args.containsKey("noteKeywords"))
                    delTarget.setNoteKeywords((String) args.get("noteKeywords"));
                if (args.containsKey("date"))
                    delTarget.setDate((String) args.get("date"));
                res.setTarget(delTarget);
                break;
            case "queryTransaction":
                res.setIntent("QUERY");
                GeminiParsedQuery query = new GeminiParsedQuery();
                if (args.containsKey("metric"))
                    query.setMetric((String) args.get("metric"));
                if (args.containsKey("type"))
                    query.setType((String) args.get("type"));
                if (args.containsKey("startDate"))
                    query.setStartDate((String) args.get("startDate"));
                if (args.containsKey("endDate"))
                    query.setEndDate((String) args.get("endDate"));
                if (args.containsKey("categoryName"))
                    query.setCategoryName((String) args.get("categoryName"));
                if (args.containsKey("minAmount"))
                    query.setMinAmount(new BigDecimal(args.get("minAmount").toString()));
                if (args.containsKey("maxAmount"))
                    query.setMaxAmount(new BigDecimal(args.get("maxAmount").toString()));
                res.setQuery(query);
                break;
            case "createBudget":
                ...
                if (args.containsKey("amount"))
                    budgetEntry.setAmount(new BigDecimal(args.get("amount").toString()));
                if (args.containsKey("categoryName"))
                    budgetEntry.setCategoryName((String) args.get("categoryName"));
                if (args.containsKey("date"))
                    budgetEntry.setDate((String) args.get("date"));
            ...
            case "createIncomeGoal":
                ...
                if (args.containsKey("amount"))
                    goalEntry.setAmount(new BigDecimal(args.get("amount").toString()));
                if (args.containsKey("categoryName"))
                    goalEntry.setCategoryName((String) args.get("categoryName"));
            ...
            case "createSchedule":
                ...
                if (args.containsKey("amount"))
                    schedEntry.setAmount(new BigDecimal(args.get("amount").toString()));
                if (args.containsKey("categoryName"))
                    schedEntry.setCategoryName((String) args.get("categoryName"));
                if (args.containsKey("repeatType"))
                    schedEntry.setRepeatType((String) args.get("repeatType"));
                if (args.containsKey("repeatConfig"))
                    schedEntry.setRepeatConfig((String) args.get("repeatConfig"));
                if (args.containsKey("walletName"))
                    schedEntry.setWalletName((String) args.get("walletName"));
                res.setEntries(List.of(schedEntry));
                break;
            case "viewFinancialPlan":
                res.setIntent("VIEW_FINANCIAL_PLAN");
                break;
            case "disableSchedule":
                res.setIntent("DISABLE_SCHEDULE");
                break;
            case "enableSchedule":
                res.setIntent("ENABLE_SCHEDULE");
                break;
            case "deleteSchedule":
                res.setIntent("DELETE_SCHEDULE");
                break;
            case "listSchedules":
                res.setIntent("LIST_SCHEDULES");
                break;
            case "getFinancialScore":
                res.setIntent("FINANCIAL_SCORE");
                break;
            case "getMonthlySummary":
                res.setIntent("MONTHLY_SUMMARY");
                break;
            case "explainTransactionSource":
                res.setIntent("EXPLAIN_TRANSACTION_SOURCE");
                break;
            case "getFinancialAdvice":
                res.setIntent("ADVICE");
                if (args.containsKey("adviceReply"))
                    res.setAdviceReply((String) args.get("adviceReply"));
                break;
            default:
    ...
    /**
     * Xây dựng chỉ dẫn hệ thống (System Prompt) cho AI.
     * Cung cấp ngữ cảnh về vai trò, danh sách hạng mục hợp lệ và các quy tắc ứng
     * xử.
     */
    ...
    /**
     * Đóng gói nội dung hội thoại: Lịch sử + Tin nhắn mới + Ảnh (nếu có).
     */
    private List<Content> buildContentsWithHistory(String message, String base64Image, List<AiMessage> history) {
        ...
        // Xử lý ảnh base64 nếu có để gửi cho AI (Multimodal)
    ...
    /**
     * Định nghĩa danh sách các "Công cụ" (Tools/Functions) mà AI có thể gọi.
     */
    private List<Tool> buildTools() {
        return List.of(
                Tool.builder().functionDeclarations(List.of(
                        // Công cụ thêm giao dịch
                        FunctionDeclaration.builder().name("insertTransaction")
                                ...
                                .parameters(Schema.builder().type(new Type(Type.Known.OBJECT))
                                        .properties(Map.of(
                                                ...
                                                "type",
                                                Schema.builder().type(new Type(Type.Known.STRING))
                                                        .description("INCOME or EXPENSE").build(),
                                                "date",
                                                Schema.builder().type(new Type(Type.Known.STRING))
                                                        .description("ISO date").build(),
                                                "walletName",
                                                Schema.builder().type(new Type(Type.Known.STRING)).build()))
                                        .required(List.of("amount", "categoryName")).build())
                                .build(),
                        // Công cụ cập nhật giao dịch
                        FunctionDeclaration.builder().name("updateTransaction")
                                ...
                                .parameters(Schema.builder().type(new Type(Type.Known.OBJECT))
                                        ...
                                        .required(List.of("amount")).build())
                                .build(),
                        // Công cụ xóa giao dịch
                        FunctionDeclaration.builder().name("deleteTransaction")
                                ...
                                .parameters(Schema.builder().type(new Type(Type.Known.OBJECT))
                                        .properties(Map.of(
                                                ...
                                                "date", Schema.builder().type(new Type(Type.Known.STRING)).build(),
                                                "deleteAll",
                                                Schema.builder().type(new Type(Type.Known.BOOLEAN)).build()))
                                        .build())
                                .build(),
                        // Công cụ truy vấn/phân tích thu chi
                        FunctionDeclaration.builder().name("queryTransaction")
                                .description("Query analytics.")
                                .parameters(Schema.builder().type(new Type(Type.Known.OBJECT))
                                        .properties(Map.of(
                                                "metric",
                                                Schema.builder().type(new Type(Type.Known.STRING)).description(
                                                        "TOTAL, TOP_CATEGORY, TREND, PERCENTAGE, WEEKLY_PATTERN, LIST")
                                                        .build(),
                                                "type",
                                                Schema.builder().type(new Type(Type.Known.STRING))
                                                        .description("EXPENSE or INCOME").build(),
                                                ...
                                                "maxAmount",
                                                Schema.builder().type(new Type(Type.Known.NUMBER)).build()))
                                        .required(List.of("metric")).build())
                                .build(),
                        // Công cụ tạo ngân sách (Budget)
                        FunctionDeclaration.builder().name("createBudget")
                                ...
                                .parameters(Schema.builder().type(new Type(Type.Known.OBJECT))
                                        .properties(Map.of(
                                                ...
                                                "date",
                                                Schema.builder().type(new Type(Type.Known.STRING))
                                                        .description("ISO month date").build()))
                                        .required(List.of("amount", "categoryName")).build())
                                .build(),
                        // Công cụ tạo mục tiêu thu nhập
                        FunctionDeclaration.builder().name("createIncomeGoal")
                                ...
                                .parameters(Schema.builder().type(new Type(Type.Known.OBJECT))
                                        ...
                                        .required(List.of("amount", "categoryName")).build())
                                .build(),
                        // Công cụ quản lý lịch trình định kỳ
                        FunctionDeclaration.builder().name("createSchedule")
                                ...
                                .parameters(Schema.builder().type(new Type(Type.Known.OBJECT))
                                        .properties(Map.of(
                                                ...
                                                "repeatType",
                                                Schema.builder().type(new Type(Type.Known.STRING))
                                                        .description("DAILY, WEEKLY, MONTHLY").build(),
                                                "repeatConfig",
                                                Schema.builder().type(new Type(Type.Known.STRING)).description(
                                                        "Day of month for MONTHLY (e.g. '5'), Day of week for WEEKLY (e.g. 'MONDAY')")
                                                        .build(),
                                                "walletName",
                                                Schema.builder().type(new Type(Type.Known.STRING)).build()))
                                        .required(List.of("amount", "categoryName", "repeatType")).build())
                                .build(),
                        ...
                        FunctionDeclaration.builder().name("explainTransactionSource")
                                .description("Explain transaction source.").build(),
                        ...
                        FunctionDeclaration.builder().name("getFinancialAdvice")
                                .description("Savings/Affordability advice.")
                                .parameters(Schema.builder().type(new Type(Type.Known.OBJECT))
                                        .properties(Map.of("adviceReply",
                                                Schema.builder().type(new Type(Type.Known.STRING)).build()))
                                        .build())
                                .build()))
                        .build());
